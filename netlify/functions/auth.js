...
// ---- session helpers, shared with profile.js / profiles.js via re-implementation ----
// (kept self-contained per-function since Netlify functions don't share modules at runtime
//  unless bundled; logic is duplicated intentionally and kept tiny)
...
async function createAdminSession(store) {
  const token = newToken();
  const expiresAt = Date.now() + SESSION_TTL_MS;
  await store.setJSON(`session:${token}`, { name: null, isAdmin: true, expiresAt });
  return token;
}
...
export const handler = async (event) => {
  ...
  if (action === "signup") {
    ...
    const existing = await store.get(accountKey, { type: "json" });
    if (existing) {
      return json(409, { error: "That name is already taken. Try logging in instead, or pick a different name." });
    }
    const pinHash = await hashPin(pin);
    await store.setJSON(accountKey, { name, pinHash, createdAt: Date.now() });

    const token = await createSession(store, name);
